...
echo !workdir! | findstr "{" >nul
if !errorlevel! equ 0 set "workdir=%cd%"
if not exist "!workdir!" set "workdir=%cd%"

:: ================== 自动请求管理员权限 ==================
net session >nul 2>&1
if %errorlevel% neq 0 (
    powershell -command "Start-Process -FilePath '%~f0' -ArgumentList '!workdir!' -Verb RunAs"
    exit /b
)
